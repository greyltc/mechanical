...
class PassthroughTestCase(unittest.TestCase):
    ...
    def test_oringer(self):
        ...
        pcb = cadquery.Assembly()
        oringer = cadquery.Assembly()

        mwp = mwp.rarray(1, 40, 1, 2).make_oringer(board_width=pt_pcb_width, board_inner_depth=inner_depth, board_outer_depth=outer_depth)

        final = cadquery.Assembly(mwp)
        final.add(hardware)
        final.add(pcb)
        final.add(oringer)
