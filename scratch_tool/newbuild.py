...
class ScratchTool(object):
    ...
    dowel_offset_from_center1 = 9
    dowel_offset_from_center2 = 4.3306
    ...
    scratch_tool_width_fudge = 0

    safe_step_z = 0.5

    outer_guide_spacing_o = 17
    outer_guide_spacing_c = 0
    ...
    def make_bottom(self, x, y):
        ...
        b = cq.CQ().add(b).faces("<Z[-1]").workplane(centerOption=co)                             .rarray(s.glass_dim+s.dowel_nominal_d+s.nominal_glass_pin_play, 2*s.dowel_offset_from_center1, 2, 2).hole(dowel_press_d).val().Solids()[0]  # cut press fit dowel pin holes
        b = cq.CQ().add(b).faces("<Z[-1]").workplane(centerOption=co).transformed(rotate=(0,0,90)).rarray(s.glass_dim+s.dowel_nominal_d+s.nominal_glass_pin_play, 2*s.dowel_offset_from_center2, 2, 2).hole(dowel_press_d).val().Solids()[0]  # cut press fit dowel pin holes
    ...
    def make_top(self, x, y):
        ...
        t = cq.CQ().add(t).faces(">Z[-1]").workplane(centerOption=co)                             .rarray(s.glass_dim+s.dowel_nominal_d+s.nominal_glass_pin_play, 2*s.dowel_offset_from_center1, 2, 2).hole(dowel_clearance_d).val().Solids()[0]  # cut clearance fit dowel pin holes
        t = cq.CQ().add(t).faces(">Z[-1]").workplane(centerOption=co).transformed(rotate=(0,0,90)).rarray(s.glass_dim+s.dowel_nominal_d+s.nominal_glass_pin_play, 2*s.dowel_offset_from_center2, 2, 2).hole(dowel_clearance_d).val().Solids()[0]  # cut clearance fit dowel pin holes
        t = cq.CQ().add(t).faces(">Z[-1]").workplane(centerOption=co).rarray(outer_guide_array_spacing_o, 1, 2, 1).rect(center_lane_width_o, y).cutBlind(-(top_z-thickness_remaining_under_slot)).val().Solids()[0]  # cut the outer slots
        t = cq.CQ().add(t).faces(">Z[-1]").workplane(centerOption=co).rarray(outer_guide_array_spacing_c, 1, 1, 1).rect(center_lane_width_c, y).cutBlind(-(top_z-thickness_remaining_under_slot)).val().Solids()[0]  # cut the central slot
