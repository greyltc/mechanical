...
class ChamberNG(object):
    ...
    # alignment pin stuff
    alp_nom = 3  # nominal diameter for alignment pins (both for substrate and for sandwitch alignment ones)
    alp_press_c = alp_nom - 0.06  # for pressfit into incomplete, crescent shaped holes
    alp_press = alp_nom - 0.035  # for pressfit into endblocks
    alp_slide = alp_nom + 0.15  # for close sliding
    alp_clear = alp_nom + 0.45  # for easy sliding
    alp_end_space = 13  # spacing between the alignment pins that are pressed into the endblocks

    # endblock stuff
    eb_wall_spacing = 0.5  # from + and - Y walls
    eb_thickness_shelf = 12
    eb_thickness_extension = 12 + pcb_cut_rad
    eb_screw_offset = 6  # distance from the endblock edge to clamp screw hole center
    eb_thickness = eb_thickness_shelf + eb_thickness_extension
    eb_locknut_depth = 5
    ...
    eb_alp_hole_depth = 15  # depth of holes for alignment pins that go into the endblock (v deep to allow for variable pin len)
    ...
    pd_half_nut_depth = 2.7  # half nut worst case depth
    pd_half_nut_d = 9  # half nut worst case diameter
    pd_m5_clearance_d = 5.8  # loose clearance for m5
    ...
    def __init__(self, array=(4, 1), subs=(30, 30), spacing=(10, 10), padding=(10, 10, 0, 0)):
        ...
        if "show_object" in globals():
            self.in_cq_editor = True
            self.sp_spacer_dxf_holes = True
        else:
            self.in_cq_editor = False
        ...
        y_minus = padding[2] + self.eb_thickness_extension  # must be at minimum eb_thickness_extension
        y_plus = padding[3] + self.eb_thickness_extension  # must be at minimum eb_thickness_extension
    ...
    def make_endblock(self):
        ...
        base = CQ().box(block_width, self.eb_thickness_shelf, self.wall_height, centered=(True, False, False))
        ...
        base = base.faces(">X[-1]").workplane(centerOption=co).center(self.eb_thickness_shelf / 2 - self.pcb_mount_hole_offset, self.wall_height / 2 - self.pcb_mount_hole_offset).circle(self.eb_mount_hole_tap_r).cutBlind(-self.eb_mount_hole_depth)
        base = base.faces("<X[-1]").workplane(centerOption=co).center(-self.eb_thickness_shelf / 2 + self.pcb_mount_hole_offset, self.wall_height / 2 - self.pcb_mount_hole_offset).circle(self.eb_mount_hole_tap_r).cutBlind(-self.eb_mount_hole_depth)
        # make crossbar pcb side alignment pin holes
        base = base.faces(">X[-1]").workplane(centerOption=co).center(-self.eb_thickness_shelf / 2 + self.pcb_mount_hole_offset, -self.wall_height / 2 + self.pcb_mount_hole_offset + self.pcb_z_float).circle(self.pcb_alignment_hole_d / 2).cutBlind(-self.pcb_alignment_hole_depth)
        base = base.faces("<X[-1]").workplane(centerOption=co).center(self.eb_thickness_shelf / 2 - self.pcb_mount_hole_offset, -self.wall_height / 2 + self.pcb_mount_hole_offset + self.pcb_z_float).circle(self.pcb_alignment_hole_d / 2).cutBlind(-self.pcb_alignment_hole_depth)

        # the endblock extension chunk that reaches furthest towards the middle
        extension_a = CQ().box(block_width, self.eb_thickness_extension, self.pcb_min_height, centered=(True, False, False))
        extension_a = extension_a.translate((0, self.eb_thickness_shelf, self.pcb_z_float + self.pcb_bottom_bump_down))

        b_width = self.pcb_bottom_bump_offset - self.eb_thickness_shelf
        ...
        extension_b = extension_b.translate((0, self.eb_thickness_shelf, self.pcb_z_float))
        ...
        endblock = (
            ...
            .center(0, self.eb_screw_offset - self.eb_thickness + self.eb_thickness_shelf / 2)
        ...
        endblock = endblock.faces(">Z[-2]").workplane(centerOption=co).center(0, self.pcb_cut_rad / 2).rarray(self.alp_end_space, 1, 2, 1).circle(self.alp_press / 2).cutBlind(-self.eb_alp_hole_depth)
    ...
    def make_endblocks(self, endblock, wp):
        ...
        endblockA = endblockA.translate((0, -self.eb_thickness_shelf + wpbb.ymin - self.extra[2], -self.wall_height + self.pcb_top_bump_up))
        endblockB = endblockB.translate((0, self.eb_thickness_shelf + wpbb.ymax + self.extra[3], -self.wall_height + self.pcb_top_bump_up))
    ...
    def make_sandwich_wires(self, wp, alignment_pin_hole_d, aux_con_hole_d, aux_con_pad_hole_d, screw_hole_r=tb.c.std_screw_threads["m5"]["clearance_r"]):
        ...
        shelf_y_minus_center = wpbb.ymin - self.extra[2] + self.eb_thickness_extension - self.eb_screw_offset
        shelf_y_plus_center = wpbb.ymax + self.extra[3] - self.eb_thickness_extension + self.eb_screw_offset
        ...
        self.shp = shp  # save these so we can use them later
        thisr = screw_hole_r
        if self.sp_spacer_dxf_holes == False:
            for x, y in shp:
                self.sp_d_vals.append(thisr * 2)
                self.sp_x_vals.append(x)
                self.sp_y_vals.append(y)
        shv = CQ().circle(thisr).extrude(1)  # volume for a single clamp screw hole
        ...
        ahpag = shg[0] - self.alp_end_space / 2  # alignment pin a x position grid
        ahpbg = shg[0] + self.alp_end_space / 2  # alignment pin b x position grid
        # fmt: off
        ...
        # fmt: on
        thisd = alignment_pin_hole_d
        if self.sp_spacer_dxf_holes == False:
            for x, y in ahp:
                self.sp_d_vals.append(thisd)
                self.sp_x_vals.append(x)
                self.sp_y_vals.append(y)
        ahv = CQ().circle(thisd / 2).extrude(one)  # volume for a single alignment pin hole
        ...
        if aux_con_hole_d > 0:
            thisd = aux_con_hole_d
            if self.sp_spacer_dxf_holes == False:
                for x, y in aehp:
                    self.sp_d_vals.append(thisd)
                    self.sp_x_vals.append(x)
                    self.sp_y_vals.append(y)
            aehv = CQ().circle(thisd / 2).extrude(one)  # for connection points from crossbars
    ...
    def make_adapter_spacer(self, wp, cpg):
        ...
        adp_spc = CQ().add(self.make_sandwich_wires(wp, self.alp_slide, self.as_aux_pin_clearance_d, self.as_aux_pad_hole_d)).toPending().extrude(self.pcb_thickness)
    ...
    def make_some_layers(self, wp, cpg):
        ...
        sp_spc = CQ().add(s.make_sandwich_wires(wp, s.alp_slide, s.sp_spacer_aux_pin_clearance_d, s.sp_spacer_aux_pad_hole_d)).toPending().extrude(s.sp_spacer_t)
        ...
        sp_spc_nh = sp_spc = CQ().add(sp_spc.findSolid())  # the spring pin spacer, pre hole cuttings
        if self.sp_spacer_dxf_holes == False:
            # conn pin holes
            for x, y in shps:
                self.sp_d_vals.append(s.sp_pin_clearance_d)
                self.sp_x_vals.append(x)
                self.sp_y_vals.append(y)
            # conn pin holes
            for x, y in spps:
                self.sp_d_vals.append(s.sp_spring_clearance_d)
                self.sp_x_vals.append(x)
                self.sp_y_vals.append(y)
        ...
        sh = CQ().add(s.make_sandwich_wires(wp, s.alp_slide, s.holder_aux_pin_clearance_d, s.holder_aux_pad_hole_d)).toPending().extrude(s.holder_t)
        ...
        pocket_extra_pin_side = s.alp_press_c / 2 * (1 - s.crescent_opening_radial_fraction_offset)
        ...
        if many_pockets == True:
            ...
            gx1 = cpg[0] - s.substrate_adapters[0] / 2 - s.alp_nom / 2
            gx2 = cpg[0] - s.substrate_adapters[0] / 2 - s.alp_nom / 2
            ...
            gy3 = cpg[1] - s.substrate_adapters[1] / 2 - s.alp_nom / 2
            gy4 = cpg[1] - s.substrate_adapters[1] / 2 - s.alp_nom / 2
            ...
            tps = c.grid2dtolist(tgx1, tgy1) + c.grid2dtolist(tgx2, tgy2)
            ...
            pcgy4 = pcgy2
            pcps = (
                # pocket corner drill rounding point list
            ...
            x1 = -s.substrate_adapters[0] * s.array[0] / 2 - s.alp_nom / 2
            x2 = -s.substrate_adapters[0] * s.array[0] / 2 - s.alp_nom / 2
            ...
            y3 = -s.substrate_adapters[1] * s.array[1] / 2 - s.alp_nom / 2
            y4 = -s.substrate_adapters[1] * s.array[1] / 2 - s.alp_nom / 2
        ...
        thisd = s.tweezer_allower_hole_d
        if self.sp_spacer_dxf_holes == False:
            for x, y in taps:
                self.sp_d_vals.append(thisd)
                self.sp_x_vals.append(x)
                self.sp_y_vals.append(y)
        tahv = CQ().circle(thisd / 2).extrude(s.sp_spacer_t)
        ...
        if self.sp_spacer_dxf_holes == False:
            for x, y in apps:
                ...
                self.sp_x_vals.append(x)
                self.sp_y_vals.append(y)
        aphv = CQ().circle(thisd / 2).extrude(s.sp_spacer_t)  # alignment pin hole volume for holes in the spring pin spacer
        aphhv = CQ().circle(s.alp_press_c / 2).extrude(s.holder_t)  # alignment pin hole volume for holes in the holder
        ...
        if self.sp_spacer_dxf_holes == False:
            for x, y in tps:
                ...
                self.sp_x_vals.append(x)
                self.sp_y_vals.append(y)
        thv = CQ().circle(thisd / 2).extrude(s.sp_spacer_t)
        ...
        thisr = s.holder_corner_r
        if self.sp_spacer_dxf_holes == False:
            for x, y in pcps:
                self.sp_d_vals.append(thisr * 2)
                self.sp_x_vals.append(x)
                self.sp_y_vals.append(y)
            print("Here are spring pin spacer hole details:")
            d_vals = self.sp_d_vals
            x_vals = self.sp_x_vals
            y_vals = self.sp_y_vals
            print(f"{d_vals=}")
            print(f"{x_vals=}")
            print(f"{y_vals=}")
        hpcv = CQ().circle(thisr).extrude(s.holder_t)
        ...
        pd = CQ().add(s.make_sandwich_wires(wp, s.alp_slide, s.pd_aux_pin_clearance_d, s.pd_aux_pad_hole_d, screw_hole_r=s.pd_m5_clearance_d / 2)).toPending().extrude(s.pd_x_side_thickness)
        ...
        # make cbore holes that make room for the half nuts
        shp = self.shp  # clamping screw hole points
        pd = pd.faces("<Z[-3]").workplane(centerOption=co).pushPoints(shp).cboreHole(diameter=1, cboreDiameter=s.pd_half_nut_d, cboreDepth=s.pd_half_nut_depth)
        ...
        apphv = CQ().circle(s.alp_clear / 2).extrude(s.pd_x_side_thickness).translate((0, 0, -s.pd_x_side_thickness + s.holder_t + s.pd_subs_alignment_z_allowance))
        ...
        return sp_spc, sp_spc_nh, sh, pd, top_pcb
    ...
    def make_crossbar(self, wp):
        ...
        crossbar_points = [  # board outline points
            ...
            (wpbb.ymax + self.extra[3] + self.eb_thickness_shelf, self.pcb_top_bump_up),
            (wpbb.ymax + self.extra[3] + self.eb_thickness_shelf, -self.pcb_min_height - self.pcb_bottom_bump_down),
            (wpbb.ymax + self.extra[3] + self.eb_thickness_shelf - self.pcb_bottom_bump_offset, -self.pcb_min_height - self.pcb_bottom_bump_down),
            (wpbb.ymax + self.extra[3] + self.eb_thickness_shelf - self.pcb_bottom_bump_offset, -self.pcb_min_height),
            (wpbb.ymin - self.extra[2] - self.eb_thickness_shelf + self.pcb_bottom_bump_offset, -self.pcb_min_height),
            (wpbb.ymin - self.extra[2] - self.eb_thickness_shelf + self.pcb_bottom_bump_offset, -self.pcb_min_height - self.pcb_bottom_bump_down),
            (wpbb.ymin - self.extra[2] - self.eb_thickness_shelf, -self.pcb_min_height - self.pcb_bottom_bump_down),
            (wpbb.ymin - self.extra[2] - self.eb_thickness_shelf, self.pcb_top_bump_up),
        ...
        crossbar_mount_hole_points = [  # locate pcb mounting holes
            ...
            (wpbb.ymax + self.extra[3] + self.eb_thickness_shelf - self.pcb_bottom_bump_offset + self.pcb_mount_hole_offset, -self.pcb_min_height - self.pcb_bottom_bump_down + self.pcb_mount_hole_offset),
            (wpbb.ymin - self.extra[2] - self.pcb_mount_hole_offset, self.pcb_top_bump_up - self.pcb_mount_hole_offset),
            (wpbb.ymin - self.extra[2] - self.eb_thickness_shelf + self.pcb_bottom_bump_offset - self.pcb_mount_hole_offset, -self.pcb_min_height - self.pcb_bottom_bump_down + self.pcb_mount_hole_offset),
        ]

        alignment_hole_point = [
            (wpbb.ymax + self.extra[3] + self.eb_thickness_shelf - self.pcb_mount_hole_offset, crossbar_mount_hole_points[1][1]),
            (wpbb.ymin - self.extra[2] - self.eb_thickness_shelf + self.pcb_mount_hole_offset, crossbar_mount_hole_points[3][1]),
        ]
        ...
        wall_end = wpbb.ymax + self.extra[2] + self.eb_thickness_shelf + self.eb_wall_spacing + self.wall[2]
        self.pcb_inch_fudge = round(math.ceil(wall_end / 1.27) * 1.27 - wall_end, 2)  # so that the inch-based connectors can be on-grid

        extension_length = self.eb_wall_spacing + self.wall[2] + self.pcb_inch_fudge + self.pcb_external_connector_width * self.array[1]
        ...
        self.crossbar_chamber_extra = extension_length - self.eb_wall_spacing - self.wall[2]
    ...
    def make_middle(self, wp):
        ...
        shelf_width = s.eb_wall_spacing + s.eb_thickness_shelf
        ...
        shelved_ring = (
            ...
            .center((s.extra[0] - s.extra[1]) / 2, +shelf_width / 2 - s.eb_thickness_shelf / 2)    # invert=True flips Y but not X
        ...
        c_len = wpbb.ylen + 2 * s.eb_thickness_shelf + s.extra[2] + s.extra[3] + s.wall[2] + s.eb_wall_spacing  # length of the slot cutter

        holes_y_spot = srbb.ymin + s.wall[2] - s.pcb_ph_remain - s.pcb_phd / 2
        # fmt: off
        _pcb_slot_void = (
            CQ().workplane(offset=-s.pcb_height)
        ...
        _pcb_slot_void = _pcb_slot_void.translate([0, -c_len + wpbb.ymax + s.extra[3] + s.eb_thickness_shelf, 0])
    ...
    def build(self):
        ...
        (spring_pin_spacer, spring_pin_spacer_no_holes, substrate_holder, pusher_downer, top_pcb) = self.make_some_layers(werkplane, cpg)
        ...
        return (asy, crossbar, adapter, adapter_spacer, spring_pin_spacer, spring_pin_spacer_no_holes, substrate_holder, top_pcb)
...
def main():
    ...
    if "show_object" in globals():  # we're in cq-editor
        ...
        if s.sp_spacer_dxf_holes == True:
            ...
            spring_pin_spacer_outline = spring_pin_spacer_no_holes.section()
            cadquery.exporters.exportDXF(spring_pin_spacer_outline, str(Path(__file__).parent / "output" / "spring_pin_spacer_outline.dxf"))
